'use client';

import { useState } from 'react';
import Image from 'next/image';
import PDFUpload from './components/PDFUpload';
import ChatInterface from './components/ChatInterface';
import { usePDFChat } from './hooks/usePDFChat';

export default function Home() {
  const [currentFile, setCurrentFile] = useState<File | null>(null);
  const { chatService, isInitializing, error, initializeChat } = usePDFChat();

  // Handle PDF file upload
  const handleFileUpload = async (file: File) => {
    setCurrentFile(file);
    await initializeChat(file);
  };

  // Handle chat message
  const handleSendMessage = async (message: string): Promise<string> => {
    if (!chatService) {
      return 'Please upload a PDF first';
    }
    
    try {
      return await chatService.askQuestion(message);
    } catch (error: any) {
      console.error('Error processing message:', error);
      throw new Error(error.message || 'Failed to process your message');
    }
  };

  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-b from-white to-gray-50 dark:from-gray-950 dark:to-gray-900 font-[family-name:var(--font-geist-sans)]">
      {/* Header */}
      <header className="w-full p-4 border-b border-gray-200 dark:border-gray-800">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-2">
            <svg
              width="32"
              height="32"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              className="text-blue-600 dark:text-blue-400"
            >
              <path
                d="M14 2H6C5.46957 2 4.96086 2.21071 4.58579 2.58579C4.21071 2.96086 4 3.46957 4 4V20C4 20.5304 4.21071 21.0391 4.58579 21.4142C4.96086 21.7893 5.46957 22 6 22H18C18.5304 22 19.0391 21.7893 19.4142 21.4142C19.7893 21.0391 20 20.5304 20 20V8L14 2Z"
                stroke="currentColor"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
              />
              <path
                d="M14 2V8H20"
                stroke="currentColor"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
              />
              <path
                d="M16 13H8"
                stroke="currentColor"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
              />
              <path
                d="M16 17H8"
                stroke="currentColor"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
              />
              <path
                d="M10 9H9H8"
                stroke="currentColor"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
              />
            </svg>
            <h1 className="text-xl font-bold">PDF AI Chat</h1>
          </div>
          
          <a
            href="https://github.com/yourusername/pdf_ai"
            target="_blank"
            rel="noopener noreferrer"
            className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300"
          >
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M12 2C6.477 2 2 6.477 2 12C2 16.991 5.57 21.128 10.43 22V18.586C10.0535 18.6764 9.67871 18.7157 9.32 18.702C8.33 18.666 7.677 18.13 7.323 17.444C7.077 16.992 6.472 16.3 6.121 16.109C5.835 15.958 5.626 15.721 5.828 15.709C6.429 15.677 6.84 16.322 6.969 16.517C7.541 17.405 8.362 17.492 9.359 17.141C9.459 16.671 9.67 16.293 9.92 16.073C7.446 15.807 6.44 14.385 6.44 12.72C6.44 11.782 6.803 10.842 7.442 10.115C7.33 9.831 7.1 9.041 7.525 7.782C7.525 7.782 8.23 7.525 9.969 8.701C10.6337 8.50048 11.3153 8.39965 12 8.4C12.69 8.4 13.38 8.5 14.031 8.701C15.77 7.516 16.476 7.782 16.476 7.782C16.9 9.041 16.67 9.831 16.559 10.115C17.197 10.842 17.561 11.773 17.561 12.72C17.561 14.394 16.546 15.807 14.071 16.073C14.393 16.347 14.68 16.878 14.68 17.693V22C19.429 21.129 23 16.992 23 12C23 6.477 18.523 2 12 2Z"
                fill="currentColor"
              />
            </svg>
          </a>
        </div>
      </header>

      {/* Main Content */}
      <main className="flex-1 px-4 py-8 max-w-7xl mx-auto w-full">
        <div className="grid md:grid-cols-2 gap-8 items-start">
          {/* Left Column - Upload and Info */}
          <div className="flex flex-col gap-6">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
              <h2 className="text-xl font-semibold mb-4">Upload Your PDF</h2>
              
              <PDFUpload onFileUpload={handleFileUpload} />
              
              {isInitializing && (
                <div className="mt-4 flex items-center justify-center">
                  <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500"></div>
                  <p className="ml-2 text-sm text-gray-600 dark:text-gray-400">Processing PDF...</p>
                </div>
              )}
              
              {error && (
                <div className="mt-4 p-3 bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-400 rounded-md text-sm">
                  {error}
                </div>
              )}
              
              {currentFile && !isInitializing && chatService && (
                <div className="mt-4 p-3 bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400 rounded-md text-sm flex items-center">
                  <svg
                    width="16"
                    height="16"
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                    className="mr-2"
                  >
                    <path
                      d="M22 11.08V12C21.9988 14.1564 21.3005 16.2547 20.0093 17.9818C18.7182 19.709 16.9033 20.9725 14.8354 21.5839C12.7674 22.1953 10.5573 22.1219 8.53447 21.3746C6.51168 20.6273 4.78465 19.2461 3.61096 17.4371C2.43727 15.628 1.87979 13.4881 2.02168 11.3363C2.16356 9.18455 2.99721 7.13631 4.39828 5.49706C5.79935 3.85781 7.69279 2.71537 9.79619 2.24013C11.8996 1.7649 14.1003 1.98232 16.07 2.85999"
                      stroke="currentColor"
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                    />
                    <path
                      d="M22 4L12 14.01L9 11.01"
                      stroke="currentColor"
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                    />
                  </svg>
                  PDF processed successfully! You can now chat with your document.
                </div>
              )}
            </div>
            
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
              <h2 className="text-xl font-semibold mb-4">How It Works</h2>
              <ol className="list-decimal list-inside space-y-3 text-sm text-gray-700 dark:text-gray-300">
                <li>Upload your PDF document</li>
                <li>Our AI will process and analyze the content</li>
                <li>Ask questions about your document in the chat</li>
                <li>Get accurate answers based on the content of your PDF</li>
              </ol>
            </div>
          </div>
          
          {/* Right Column - Chat Interface */}
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
            <h2 className="text-xl font-semibold mb-4">Chat with Your PDF</h2>
            <ChatInterface 
              onSendMessage={handleSendMessage} 
              isReady={!!chatService && !isInitializing} 
            />
          </div>
        </div>
      </main>

      {/* Footer */}
      <footer className="w-full p-6 border-t border-gray-200 dark:border-gray-800 mt-auto">
        <div className="max-w-7xl mx-auto text-center text-sm text-gray-600 dark:text-gray-400">
          <p>PDF AI Chat - Powered by Next.js, LangChain, and OpenAI</p>
          <p className="mt-1">© {new Date().getFullYear()} Your Company</p>
        </div>
      </footer>
    </div>
  );
}